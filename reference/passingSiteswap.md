...
- `@is_fractional`:
  ...
  `TRUE` for fractional notation; `FALSE` for p-notation.

- `@n_jugglers`:

  Number of jugglers (sections in the sequence).
...
- `@throws_by_juggler`:
  ...
  for p-notation; may include `.5` for fractional).
...
- `@period`:
  ...
  Number of throws per juggler per cycle.
...
- `@n_props`:
  ...
  Total number of props (`sum(all throws) / period`).
