...
Jugglers have a notation for their patterns. A 3-ball cascade — the first pattern most jugglers learn — is written as `3`. Five balls is `5`. That trick where one ball sails high while the other two move beneath it is `423`. The notation is called siteswap, and each number encodes how many beats until that prop needs to come back down. jugglr lets you create, validate, and visualise siteswap patterns in R.
